...
Page
{
    ...
    // Close loader(dialog) & keyboard when user swipes away.
    onFocusChanged:
    {
        if (id_swipe_view.currentIndex == page_chat)
            inactivate();
    }
    ...
    Rectangle
    {
        ...
        Text
        {
            ...
            text: Number.fromLocaleString(Chat.size);
            verticalAlignment: Text.AlignVCenter;
            width: parent.width;
        }
        ...
        ListView
        {
            ...
            delegate: Loader
            {
                ...
                source: "qrc:/qml/delegate/ChatDelegate.qml";
            }
            focus: true;
            height: parent.height;
            ...
            Keys.onPressed:
            {
                if ((event.key == Qt.Key_Enter) || (event.key == Qt.Key_Return))
                {
                    if (Chat.enabled())
                    {
                        ...
                        {
                            var result = Chat.add(id_input.text);
                            if (result)
                                id_input.text = "";
                        }
                    }
